...
Attention Mechanism Basics                                                    
  ...
  For sequence length n, self-attention computes:                               
  ...
  ┌─────────────┬──────────────────┬──────────────────┬─────────────────┐       
  │ Context (n) │ Attention Matrix │ Relative Compute │ Relative Memory │       
  ├─────────────┼──────────────────┼──────────────────┼─────────────────┤       
  │ 4K          │ 16 million       │ 1×               │ 1×              │       
  ├─────────────┼──────────────────┼──────────────────┼─────────────────┤       
  │ 8K          │ 64 million       │ 4×               │ 4×              │       
  ...
  │ 32K         │ 1 billion        │ 64×              │ 64×             │       
  ├─────────────┼──────────────────┼──────────────────┼─────────────────┤       
  │ 128K        │ 16 billion       │ 1,024×           │ 1,024×          │       
  ├─────────────┼──────────────────┼──────────────────┼─────────────────┤       
  │ 200K        │ 40 billion       │ 2,500×           │ 2,500×          │       
  └─────────────┴──────────────────┴──────────────────┴─────────────────┘       
  ---                                                                           
  Real VRAM Usage (KV Cache)                                                    
                                                                                
  The KV cache stores keys/values for all previous tokens. This is the main     
  memory hog:                                                                   
                                                                                
  KV cache size = 2 × n × d_model × layers × bytes_per_param                    
                                                                                
  Llama 3 70B example (d=8192, 80 layers, fp16):                                
  ┌─────────┬───────────────┬─────────────────┬────────────┐                    
  │ Context │ KV Cache Only │ + Model Weights │ Total VRAM │                    
  ├─────────┼───────────────┼─────────────────┼────────────┤                    
  │ 4K      │ 10 GB         │ 140 GB          │ ~150 GB    │                    
  ├─────────┼───────────────┼─────────────────┼────────────┤                    
  │ 8K      │ 21 GB         │ 140 GB          │ ~161 GB    │                    
  ├─────────┼───────────────┼─────────────────┼────────────┤                    
  │ 32K     │ 84 GB         │ 140 GB          │ ~224 GB    │                    
  ├─────────┼───────────────┼─────────────────┼────────────┤                    
  │ 128K    │ 336 GB        │ 140 GB          │ ~476 GB    │                    
  └─────────┴───────────────┴─────────────────┴────────────┘                    
  Your local GPU: 8-24 GB typically                                             
  One H100: 80 GB                                                               
  128K context on 70B: needs ~6× H100s just for memory                          
                                                                                
  ---                                                                           
  Time Per Token                                                                
                                                                                
  Each generated token must attend to ALL previous tokens:                      
                                                                                
  Token 1:     attend to 1 token      → 1 unit                                  
  Token 1000:  attend to 1000 tokens  → 1000 units                              
  Token 100K:  attend to 100K tokens  → 100,000 units                           
  ┌─────────┬───────────────────────────┬────────────────────┐                  
  │ Context │ Time per token (relative) │ 1000 tokens output │                  
  ├─────────┼───────────────────────────┼────────────────────┤                  
  │ 4K      │ 1×                        │ 4 seconds          │                  
  ├─────────┼───────────────────────────┼────────────────────┤                  
  │ 32K     │ 8×                        │ 32 seconds         │                  
  ...
  └─────────┴───────────────────────────┴────────────────────┘                  
  ---                                                                           
  Why ChatGPT/Claude Servers "Solve" It                                         
                                                                                
  They don't solve it — they brute-force it with hardware:                      
  Column 1: GPUs                                                                
  Your PC: 1× RTX 4090 (24GB)                                                   
  OpenAI/Anthropic: 1000s of H100s (80GB each)                                  
  ────────────────────────────────────────                                      
  Column 1: Total VRAM                                                          
  Your PC: 24 GB                                                                
  OpenAI/Anthropic: 100+ TB                                                     
  ────────────────────────────────────────                                      
  Column 1: Cost                                                                
  Your PC: $1,600 one-time                                                      
  OpenAI/Anthropic: $millions/month                                             
  ────────────────────────────────────────                                      
  Column 1: Parallelism                                                         
  Your PC: None                                                                 
  OpenAI/Anthropic: Tensor parallel across 8-64 GPUs per request                
  They also use optimizations:                                                  
  - Flash Attention: O(n) memory instead of O(n²) (but compute still O(n²))     
  - PagedAttention: Dynamic memory allocation                                   
  - KV cache compression: Quantization, eviction policies                       
  - Batching: Serve many users simultaneously                                   
                                                                                
  But they still have limits:                                                   
  - GPT-4: 128K max                                                             
  - Claude: 200K max                                                            
  - Gemini: 1M (with heavy optimization + quality tradeoffs)                    
                                                                                
  ---                                                                           
  The Core Point                                                                
                                                                                
  Local 24GB GPU:                                                               
    - 70B model at 4K context: barely fits                                      
    - 70B model at 32K context: impossible                                      
                                                                                
  Cloud with 8× H100 (640GB):                                                   
